...
class Message extends BaseCollection {
    ...
    async delete() {
        ...
        let deleteMessage = await global.rest.delete(`/rooms/${this.room.id}/messages/${this.id}`);
        return deleteMessage;
    }

    async edit(content) {
        // Edit the message
        let editMessage = await global.rest.patch(`/rooms/${this.room.id}/messages/${this.id}`, { data: { content } });
        return editMessage;
    }
}
